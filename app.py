...
st.markdown(
    ...
    .stSelectbox > div > div { background-color: #e8f5e8; }
...
# ============================================================
# UI Languages (English/Marathi)
ui_languages = {
    "English": "en",
    "मराठी": "mr"
}

# Disease Detection Languages (10 languages)
detection_languages = {
    "English": "en",
    "Hindi": "hi",
    "Marathi": "mr",
...
with st.sidebar:
    ...
    # UI Language selection (moved to top)
    ...
    # Get current UI language translations
    ...
    page = st.radio(
        ...
        index=0 if st.session_state.page == 'Home' else (1 if st.session_state.page == 'About Us' else 2),
        key="page_nav"
    )
    
    # Map translated page names back to English
    if page == t['home']:
        st.session_state.page = 'Home'
    elif page == t['about']:
    ...
    # Disease Detection Language selection
...
if st.session_state.page == 'Home':
    # Add spacing and display title with better formatting
    st.markdown("<div style='margin-top: 30px;'></div>", unsafe_allow_html=True)
    
    # Better title formatting for Marathi text
    if st.session_state.language == 'marathi':
        st.markdown(f"<h1 style='margin-top: 20px; font-size: 2.2rem; line-height: 1.3;'>{t['title']}</h1>", unsafe_allow_html=True)
    else:
        st.markdown(f"<h1 style='margin-top: 20px;'>{t['title']}</h1>", unsafe_allow_html=True)
    
    st.markdown(f"<p class='subtext'>{t['subtitle']}</p>", unsafe_allow_html=True)
    st.divider()

    # Upload & capture section (responsive columns)
    col1, col2 = st.columns([1, 1] if st.session_state.get('mobile', False) else 2)
    with col1:
    ...
    if image:
        ...
        if st.button(t['diagnose']):
            with st.spinner(t['analyzing']):
                ...
                st.info(f"**{t['disease_detected_lang']} ({detection_lang_option}):** {translated_disease_text}")
                ...
                st.markdown(f"### {t['suggestion_lang']} ({detection_lang_option})")
